...
import { provisionPaidTenant } from "./tenant-provisioning.ts";

// Lógica de processamento de evento do Asaas, compartilhada entre o
// webhook (asaas-webhook) e o reprocessamento manual (reprocessar-webhook-
// evento, painel da Plataforma) — mesma função, dois pontos de entrada,
// pra nunca haver duas implementações divergentes do mesmo efeito.
...
// Núcleo da Fase 4/6/8: nunca confia cegamente no payload — sempre resolve
...
async function ativarPagamento(admin: AdminClient, payment: AsaasPayment) {
  ...
  // numa renovação) evita colidir com esse índice e uma segunda linha
  // nunca seria criada mesmo.
  ...
  // desconto_aplicado é sempre derivado (preço de tabela - o que foi
  // cobrado de fato), nunca lido do payload — o valor cobrado já veio
  // com o cupom aplicado desde a criação do checkout (Fase 8).
  const statusPagamento = payment.status === "RECEIVED" ? "recebido" : "confirmado";
  const descontoAplicado = typeof valorBase === "number" ? Math.max(0, valorBase - payment.value) : 0;
  const pagamentoRow = {
    empresa_id: empresaId,
    checkout_intencao_id: intencao.id,
    provider: "asaas",
  ...
  // Confirma a reserva de cupom (se houver) — incrementa utilizacoes_atual
  // de forma atômica. No-op se não houver cupom reservado pra esta intenção.
  if (!jaProcessado) {
    const { error: cupomErr } = await admin.rpc("confirmar_cupom_checkout", { p_checkout_intencao_id: intencao.id });
    if (cupomErr) console.error("Falha ao confirmar cupom (não bloqueia a ativação)", cupomErr);
  }
...
export async function processarEventoAsaas(admin: AdminClient, eventType: string, body: AsaasWebhookBody): Promise<void> {
  switch (eventType) {
