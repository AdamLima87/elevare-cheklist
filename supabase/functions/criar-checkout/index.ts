...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) return jsonError(corsHeaders, "Não autenticado.", 401);
    const token = authHeader.replace("Bearer ", "");
    const {
      data: { user },
      error: userError,
    } = await admin.auth.getUser(token);
    if (userError || !user) return jsonError(corsHeaders, "Sessão inválida.", 401);
    ...
    let { data: intencao, error: intencaoError } = await admin
      ...
      .limit(1)
      .maybeSingle();
    if (intencaoError) return jsonError(corsHeaders, "Erro ao buscar contratação pendente.", 500);
    ...
    if (!intencao) {
      ...
      if (!periodicidade) {
        return jsonError(corsHeaders, "Nenhuma contratação pendente encontrada para esta conta.", 404);
      }
      ...
      const { data: profile } = await admin
        ...
        .eq("id", user.id)
        .maybeSingle();
      if (!profile?.empresa_id) return jsonError(corsHeaders, "Conta sem empresa associada.", 400);
      ...
      const { data: novaIntencao, error: novaIntencaoError } = await admin
        ...
        .select("id, plano_codigo, periodicidade, provider_checkout_id, checkout_url, status, updated_at, origem")
        .single();
      if (novaIntencaoError || !novaIntencao) return jsonError(corsHeaders, "Não foi possível iniciar a contratação.", 500);
    ...
    if (limitesError || !limites) return jsonError(corsHeaders, "Não foi possível resolver o preço do plano.", 500);
    ...
    if (body.cupomCodigo) {
      ...
      if (cupomError || !cupomAplicado) {
        return jsonError(corsHeaders, cupomError?.message || "Não foi possível aplicar o cupom.", 400);
      }
    ...
    if (intencao.periodicidade === "mensal") {
      if (typeof precoMensal !== "number") return jsonError(corsHeaders, "Preço do plano mensal não configurado.", 500);
      ...
      if (typeof precoAnual !== "number") return jsonError(corsHeaders, "Preço do plano anual não configurado.", 500);
      ...
      return jsonError(corsHeaders, "Periodicidade da intenção de checkout inválida.", 500);
    }
    ...
    const { error: updateError } = await admin
      ...
      .eq("id", intencao.id);
    if (updateError) return jsonError(corsHeaders, "Checkout criado, mas não foi possível salvar. Tente novamente.", 500);
    ...
    return jsonError(corsHeaders, "Não foi possível iniciar o pagamento. Tente novamente.", 500);
  }
});
