...
serve(async (req) => {
  ...
  try {
    ...
    if (profile?.perfil !== "super_admin") return jsonError(corsHeaders, "Acesso restrito à administração da plataforma.", 403);

    const { eventoId } = (await req.json().catch(() => ({}))) as { eventoId?: string };
    if (!eventoId) return jsonError(corsHeaders, "eventoId é obrigatório.", 400);
    ...
    const { data: evento, error: eventoError } = await admin
      ...
      .eq("id", eventoId)
      .maybeSingle();
    if (eventoError || !evento) return jsonError(corsHeaders, "Evento não encontrado.", 404);
    ...
    try {
      ...
      return jsonError(corsHeaders, `Falha ao reprocessar: ${message}`, 500);
    }
  } catch (err) {
    console.error("Erro inesperado em reprocessar-webhook-evento", err);
    return jsonError(corsHeaders, "Não foi possível reprocessar o evento.", 500);
  }
});
