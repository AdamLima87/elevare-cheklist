-- Fase B — liga de verdade crm_comercial_config.ganha_exige ao fechamento de
-- oportunidade como Ganha (a tabela existia desde a Fase A, mas era só
-- decorativa até aqui). Mesma assinatura (uuid, text DEFAULT NULL) — CREATE
-- OR REPLACE simples, sem necessidade de DROP (só muda o corpo, não os
-- argumentos). Checagem nova entra depois da checagem de Diagnóstico já
-- existente, seguindo o mesmo idioma (guarda + RAISE EXCEPTION com prefixo
-- 'CODIGO: '). Escopada estritamente à própria oportunidade — um contrato
-- assinado de OUTRA oportunidade da mesma Conta nunca satisfaz o gating.
-- Não revalida oportunidades já fechadas (o caminho de idempotência acima
-- retorna antes de chegar nesta checagem).
...
BEGIN
  ...
  -- Fase B: gating comercial — proposta aceita sempre exigida; contrato
  -- assinado exigido também quando ganha_exige='contrato_assinado'. Sem
  -- linha em crm_comercial_config (não deveria acontecer, sempre semeada),
  -- assume o default 'proposta_aceita'. Ambas as checagens são escopadas à
  -- PRÓPRIA oportunidade (crm_oportunidade_id), nunca a outra oportunidade
  -- da mesma Conta.
  SELECT ganha_exige INTO v_ganha_exige FROM public.crm_comercial_config WHERE empresa_id = v_oportunidade.empresa_id;
  v_ganha_exige := COALESCE(v_ganha_exige, 'proposta_aceita');

  SELECT EXISTS (
    SELECT 1 FROM public.crm_propostas p
    WHERE p.crm_oportunidade_id = p_oportunidade_id AND p.status = 'aceita'
  ) INTO v_proposta_aceita;

  IF NOT v_proposta_aceita THEN
    RAISE EXCEPTION 'PROPOSTA_NAO_ACEITA: esta oportunidade não tem nenhuma proposta comercial aceita.';
  END IF;

  IF v_ganha_exige = 'contrato_assinado' THEN
    SELECT EXISTS (
      SELECT 1 FROM public.crm_contratos c
      WHERE c.crm_oportunidade_id = p_oportunidade_id AND c.status = 'assinado'
    ) INTO v_contrato_assinado;

    IF NOT v_contrato_assinado THEN
      RAISE EXCEPTION 'CONTRATO_NAO_ASSINADO: esta oportunidade exige um contrato assinado antes de ser fechada como ganha.';
    END IF;
  END IF;
